...
## User-reported immobile start: seed 2124797507

The 50,000-evaluation failure on ridge seed 2124797507 (search seed 881497) is provably unsolvable under this simulator's action model. At its initial pose, pitch is 20.8121°, grip is 0.88, full-throttle acceleration is capped at 3.3 m/s², and opposing gravity is 3.4855 m/s². The speed update clamps to [0,9], so the truck cannot reverse or roll backward. At zero speed, yaw and translation are zero regardless of steering. The starting pose is therefore invariant under every bounded control sequence.

Full-throttle probes with left/straight/right steering all timed out at zero distance. A varying-command regression also retains zero movement. Reproduce with `npm run challenge:diagnose -- 2124797507 ridge`; the record is `output/challenge/diagnosis-2124797507.json`. This is a post-hoc diagnosis of the reported failure, not an unseen success result.

New challenge manifests record `startCheck: "immobile-start-v1"`. Both arms run the same narrow analytic check and report an **infeasible start** with zero synthesis evaluations when this invariant applies. The terrain is retained as unsuccessful in the ledger and denominator; it is never replaced with a favorable seed. Check wall time is recorded separately. Passing the check does not certify reachability or controller existence. Historical manifests and their original counts remain unchanged, and removal ablations preserve the original check setting.

No physics parameters, DSL acceptance criteria, learned prior, or controller have been changed by this diagnostic. It prevents pointless search on one known task/model mismatch. It does not fix the forward-only vehicle abstraction, solve the outer DSL-search problem, or provide evidence of a learned-language advantage.
