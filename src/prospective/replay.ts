...
export function synthesizeReplay(
  task: Task,
  macros: Macro[],
  ...
  const seen = new Set<string>(),
    population = 32;
  // Replay is a learned search proposal source, not an added DSL primitive.
  // Every replayed program is scored and charged inside the proposal budget.
  const replay = corpus.map((c) => rewrite(c.tree, macros));
  const macroCount = (e: Expr): number =>
    Number(!BASE.includes(e.op) && e.op !== "arg" && e.op !== "const") +
    e.args.reduce((s, a) => s + macroCount(a), 0);
  const score = (tree: Expr): Candidate => {
  ...
  for (const tree of replay) {
    if (count >= budget) break;
    pool.push(score(tree));
    if (best!.error < 1e-8) break;
  }
  pool.sort((a, b) => a.error - b.error || a.size - b.size);
  pool = pool.slice(0, population);
  while (count < budget && (!best || (best as Candidate).error >= 1e-8)) {
    ...
    while (batch.length < population && count < budget) {
      ...
      if (!pool.length || rng.next() < 0.3) tree = random(2 + rng.int(3));
        ...
        if (rng.next() < 0.45) {
          const leaves = paths(parent).filter(
              (p) => at(parent, p).args.length === 0,
            ),
            chosen = rng.pick(leaves),
            old = at(parent, chosen);
          tree = replace(
            parent,
            chosen,
            old.op === "const"
              ? { op: "const", args: [], value: rng.pick([-2, -1, 0, 1, 2]) }
              : { op: "arg", args: [], value: 1 - old.value! },
          );
        } else
          tree = replace(
            parent,
