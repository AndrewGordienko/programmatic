...
const folder = "output/joint/inner-refinement-development-v1";
...
const common = { localAffineNeighbors: 6, lattice: true };
const relations = {
  ...common,
...
const votes = { ...common, affineVoting: true, affineVoteIndependent: true };
const variants: {
  name: string;
  policyKind: "original" | "visited";
  options: Parameters<typeof inverseSearch>[5];
}[] = [
  { name: "original", policyKind: "original", options: common },
  { name: "visited-constraints", policyKind: "visited", options: relations },
  {
    name: "visited-share25",
    policyKind: "visited",
    options: { ...relations, branchShare: 0.25 },
  },
  { name: "votes", policyKind: "original", options: votes },
  {
    name: "visited-votes",
    policyKind: "visited",
    options: { ...relations, ...votes },
  },
  {
    name: "visited-votes-unary",
    policyKind: "visited",
    options: { ...relations, ...votes, latticeOrder: "after-unary" },
  },
];
const sourceFiles = [
  "scripts/inner-refinement-development.ts",
...
const protocol = {
  version: "inner-refinement-development-v1",
  seed: 62492731,
  tasks: 500,
  reps: [0, 7],
  budget: 512,
  ...
  note: "Fresh DEVELOPMENT, not final evidence. 500 functions, 60/20/20 related/nested/longer, two optimizer seeds, three preselected libraries including base. Previously inspected task functions, including all eight lattice-evolution runs, the 20,000-task downstream stream and both previous fresh inner suites, plus prior roots/subtrees and critic/visited-policy exposures are excluded empirically. Compare variants by equally weighted mean work-AUC across structural groups and libraries; all results retained. Point arithmetic, process CPU and elapsed wall remain separate from heterogeneous structural/program work. Selection is exclusively balanced work-AUC, not a desired language advantage. Any selected variant requires a new frozen confirmation/final study. No oracle library participates.",
};
...
const tasks = makeTasks(
  { training: 5, development: 5, confirmation: 5, testing: 500 },
  {
    seed: protocol.seed,
    exclude: excluded,
    prefix: "inner-refinement-development",
