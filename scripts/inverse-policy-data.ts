...
import { specificationFeatures } from "../src/joint/specification";
...
for (const program of source.programs) {
  ...
  for (const library of [[], source.library]) {
    ...
    const split =
      program.source === "dream" && program.id % 10 === 0
        ? "validation"
        : "training";
    let e = tree,
      partial = hole(),
  ...
  JSON.stringify(
    {
      ...
      note: "Paired base/library representations of executed programs. Teacher traverses the rightmost unknown child with actual inverse constraints and known sibling programs. Both variants of a source program remain in one train/validation split. Legal-production masks remove unavailable macro actions. Final tasks are not used; exposure audit must include all recorded teacher subprograms.",
    },
    null,
    2,
  ),
