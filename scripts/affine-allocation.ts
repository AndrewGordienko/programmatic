...
const out = "output/joint/affine-allocation-v1.json";
...
const arms: {
  ...
  { name: "l1", policy, options: { ...common, affineStrategy: "l1" } },
  {
    name: "constraints",
    policy,
    options: { ...common, affineStrategy: "constraints" },
  },
  {
    name: "constraints-64",
    policy,
    options: { ...common, affineStrategy: "constraints", affineHoleBudget: 64 },
  },
  {
    name: "constraints-32",
    policy,
    options: { ...common, affineStrategy: "constraints", affineHoleBudget: 32 },
  },
  {
    name: "constraints-128",
    policy,
    options: {
      ...common,
      affineStrategy: "constraints",
      affineHoleBudget: 128,
    },
  },
  {
    name: "visited-constraints-64",
    policy: visited,
    options: {
      ...common,
      affineStrategy: "constraints",
...
writeFileSync(
  out,
  JSON.stringify({
    note: "Adaptive calibration, not final evidence. Bounded affine coefficients searched in L1 order; two exact equalities may eliminate one coefficient; per-hole caps are compared. Fixed learned languages are old v4 and replication7, no oracle concepts. All tasks adaptive calibration only, not final evidence. All complete executions and inverse/fitting work charged; point comparisons and wall time also reported. No oracle syntax or hidden checks guide search.",
    rows,
  }),
);
