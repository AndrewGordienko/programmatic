...
const out = "output/joint/affine-voting-v2.json";
...
const arms: {
  ...
  { name: "votes-independent", policy, options: votes },
  { name: "votes-12", policy, options: { ...votes, affineVoteLimit: 12 } },
  { name: "votes-24", policy, options: { ...votes, affineVoteLimit: 24 } },
  {
    name: "visited-votes-12",
    policy: visited,
    options: {
      ...votes,
      affineVoteLimit: 12,
...
writeFileSync(
  out,
  JSON.stringify({
    note: "Adaptive calibration, not final evidence. Integer affine slopes enumerated and intercepts inferred by voting on supplied outputs. Every retained affine fragment is executed and charged. Require three non-collinear supporting inputs; optionally retain only the strongest 12 or 24 fits. Compare against previous random local-triple proposals. The flooded-bank v1 failure is preserved separately. Fixed learned languages are old v4 and replication7, no oracle concepts. All tasks adaptive calibration only, not final evidence. All complete executions and inverse/fitting work charged; point comparisons and wall time also reported. No oracle syntax or hidden checks guide search.",
    rows,
  }),
);
