...
const out = "output/joint/hole-tree-entry-v2.json";
...
const tree: HoleValueModel = JSON.parse(
  readFileSync("output/joint/neural-visited-v2/tree8/model.json", "utf8"),
);
...
for (const [m, folder] of [
  ...
  for (const fixture of JSON.parse(
    readFileSync(
      `output/joint/neural-visited-v2/${folder}/parity.json`,
      "utf8",
    ),
  ))
    assert.ok(
...
const arms = [
  ...
  {
    name: "tree-half",
    options: { lazyHoleFeatures: true, holeValue: tree, holeValueWeight: 0.5 },
  },
  {
    name: "tree-one",
    options: { lazyHoleFeatures: true, holeValue: tree, holeValueWeight: 1 },
  },
  {
    name: "forest-half",
    options: {
  ...
  {
    name: "forest-one",
    options: { lazyHoleFeatures: true, holeValue: forest, holeValueWeight: 1 },
  },
];
const rows = [];
for (const arm of arms)
  for (const learned of [false, true]) {
    ...
    const summary = {
      ...
      criticFeatures: trials.reduce(
        (s, t) => s + t.result.holeValueFeatures,
        0,
      ),
