...
const out = "output/joint/compiled-relations-v1.json";
...
const arms = [
  ...
  { name: "compiled", options: { compiledRelations: true } },
  {
    name: "compiled-more-bindings",
    options: { compiledRelations: true, macroBindings: 24 },
  },
  {
    name: "compiled-unary",
    options: { compiledRelations: true, unaryFirst: 0.4 },
  },
];
const rows = [];
...
writeFileSync(
  out,
  JSON.stringify({
    note: "Adaptive calibration only. Cached symbolic derivations for repeated partial-application arguments. Complete candidates and actual new derivations charged; semantic point checks remain counted separately. Cache-key construction overhead is included in wall time.",
    rows,
  }),
);
