...
const out = "output/joint/literal-bindings-v1.json";
...
const arms = [
  ...
  {
    name: "literals",
    options: { literalBindings: true, compiledRelations: true },
  },
  {
    name: "literals-relations",
    options: {
...
writeFileSync(
  out,
  JSON.stringify({
    note: "Adaptive calibration, not final evidence. Literal bindings to every multi-argument macro are considered before error-ranked fragments at the same binding width. Optional hole-local fragments separately diagnosed. All complete executions and inverse/fitting work charged; point comparisons and wall time also reported. No oracle syntax or hidden checks guide search.",
    rows,
  }),
);
