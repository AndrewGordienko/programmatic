...
const out = "output/joint/fragment-construction-v1.json";
...
const arms = [
  ...
  { name: "lattice32", options: { affineLattice: 32 } },
  { name: "lattice64", options: { affineLattice: 64, forwardFraction: 0.7 } },
  {
    name: "lattice64-sort",
    options: { affineLattice: 64, forwardFraction: 0.7, sortForward: true },
  },
  {
    name: "lattice32-inverse",
    options: { affineLattice: 32, relational: true },
  },
  {
    name: "lattice64-join",
    options: {
...
writeFileSync(
  out,
  JSON.stringify({
    note: "Adaptive calibration only. Generic bounded-integer affine enumeration, not hidden generator components. All complete candidates charged; lattice coefficient sorting adds overhead captured in wall time but not structural counters.",
    rows,
  }),
);
