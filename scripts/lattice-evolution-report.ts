...
type MeasuredResult = InverseResult & { cpuMicros: number };
type Trial = {
  task: string;
  signature: string;
  group: string;
...
type Run = {
  ...
  cost: {
    ...
    discoveryCpuMicros: number;
    incrementalConstraintPoints: number;
...
const metrics = {
  solve: (r: MeasuredResult) => Number(r.solved),
  workAuc: (r: MeasuredResult) =>
    r.solved ? 1 - r.work / (r.budget + r.expansionBudget) : 0,
  work: (r: MeasuredResult) => r.work,
  wallMs: (r: MeasuredResult) => r.elapsedMs,
  cpuMicros: (r: MeasuredResult) => r.cpuMicros,
  constraintPoints: (r: MeasuredResult) => r.constraintPoints,
  proposals: (r: MeasuredResult) => r.evaluations,
};
const comparisons = [
...
for (const run of runs) {
  ...
  for (const trial of run.trials) {
    if (!Number.isFinite(trial.result.cpuMicros) || trial.result.cpuMicros < 0)
...
const score = (
  ...
  arm: string,
  group: string,
  metric: (r: MeasuredResult) => number,
) => {
...
const perSeed = runs.map((run) => {
  ...
  const cpuSaving =
    score(run, "base-guided", "all", metrics.cpuMicros) -
    score(run, "accepted-guided", "all", metrics.cpuMicros);
  return {
    seed: run.seed,
    ...
    incrementalBreakEven: {
      cpu:
        cpuSaving > 0
          ? Math.ceil(run.cost.discoveryCpuMicros / cpuSaving)
          : null,
      work:
        workSaving > 0
          ? Math.ceil(
  ...
  gates: complete
    ? {
        ...
        confirmedLanguageUtility:
          runs.filter((r) => r.accepted).length >=
            Math.ceil(protocol.config.seeds.length * 0.8) &&
          perSeed.filter((s) => s.workAucDelta > 0).length >=
            Math.ceil(protocol.config.seeds.length * 0.8) &&
    ...
    : null,
  perSeed,
  note: "Meta-seed bootstrap intervals conditional on one shared pretrained policy and this task distribution; optimizer repetitions are averaged. Partial reports do not establish robustness. Across-run repeated task functions are disclosed. Rejected candidates remain in candidate comparisons; accepted-policy comparison falls back to base. Work/proposal/CPU/wall deltas use candidate minus baseline, so negative means saving. Incremental break-even is an extrapolation from per-task means, not observed amortization; heterogeneous work is not CPU-equivalent. Process CPU is separately measured for every search and whole discovery, excluding sleep time; extra point arithmetic is also retained. The inner solver changed after independent confirmation, so these runs do not replace the previous 20-run benchmark. Shared prior cost cancels only in the matched-prior comparison; uniform baseline comparisons must include it. Historical R&D and global protocol setup are not included in incremental run timing.",
};
