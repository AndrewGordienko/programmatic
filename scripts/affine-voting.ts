...
const out = "output/joint/affine-voting-v1.json";
...
const arms: {
  ...
  { name: "votes", policy, options: { ...common, affineVoting: true } },
  {
    name: "votes-relations",
    policy,
    options: {
      ...common,
      affineVoting: true,
...
writeFileSync(
  out,
  JSON.stringify({
    note: "Adaptive calibration, not final evidence. Integer affine slopes enumerated and intercepts inferred by voting on supplied outputs. Every retained affine fragment is executed and charged. Compare against previous random local-triple proposals. Fixed learned languages are old v4 and replication7, no oracle concepts. All tasks adaptive calibration only, not final evidence. All complete executions and inverse/fitting work charged; point comparisons and wall time also reported. No oracle syntax or hidden checks guide search.",
    rows,
  }),
);
