...
const out = "output/joint/fragment-calibration-v1.json";
...
const arms = [
  ...
  { name: "rank", options: { fragmentValue: model } },
  { name: "balanced", options: { fragmentValue: model, fragmentPreserve: 32 } },
  {
    name: "operands",
    options: { fragmentValue: model, fragmentPreserve: 32, fragmentWeight: 1 },
  },
];
const rows = [];
...
writeFileSync(
  out,
  JSON.stringify({
    note: "Adaptive calibration only, with v4 library and 75 observations. Teacher classification accuracy and search effects are separate. Neural fragment ranking adds real work not captured by old structural counters; wall time includes it and is observational on a shared host.",
    rows,
  }),
);
