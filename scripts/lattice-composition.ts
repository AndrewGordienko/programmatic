...
const out = "output/joint/lattice-composition-v1.json";
...
const arms = [
  ...
  { name: "lattice", options: { lattice: true } },
  {
    name: "lattice-local6",
    options: { lattice: true, localAffineNeighbors: 6 },
  },
  {
    name: "lattice-local12",
    options: { lattice: true, localAffineNeighbors: 12 },
  },
];
const rows = [];
...
writeFileSync(
  out,
  JSON.stringify({
    note: "Adaptive calibration, not final evidence. Generic scalar piecewise-affine heuristic: min/max covers of affine pieces estimated from supplied examples. No hidden AST, concept template, or final-check feedback enters synthesis. Every cover candidate is executed and charged. Both arms receive the identical heuristic. All complete executions and inverse/fitting work charged; point comparisons and wall time also reported. No oracle syntax or hidden checks guide search.",
    rows,
  }),
);
