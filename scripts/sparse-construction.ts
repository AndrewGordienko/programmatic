...
const out = "output/joint/sparse-construction-v1.json";
...
const arms = [
  ...
  { name: "sparse2", options: { sparseWidth: 2 } },
  { name: "sparse4", options: { sparseWidth: 4, sparseSteps: 2048 } },
  {
    name: "lattice-sparse",
    options: {
      sparseWidth: 4,
...
writeFileSync(
  out,
  JSON.stringify({
    note: "Adaptive calibration only. Sparse integer composition of executed fragments, not hidden generator components. All complete candidates charged; lattice coefficient sorting adds overhead captured in wall time but not structural counters.",
    rows,
  }),
);
