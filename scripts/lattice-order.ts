...
const out = "output/joint/lattice-order-v1.json";
...
const votes: Parameters<typeof inverseSearch>[5] = {
  ...common,
  affineVoting: true,
...
const arms: {
  ...
  { name: "votes-before", policy, options: votes },
  {
    name: "votes-after-unary",
    policy,
    options: { ...votes, latticeOrder: "after-unary" },
  },
  {
    name: "votes-after-forward",
    policy,
    options: { ...votes, latticeOrder: "after-forward" },
  },
  { name: "votes-no-cover", policy, options: { ...votes, lattice: false } },
  {
    name: "local-after-unary",
    policy,
    options: { ...common, latticeOrder: "after-unary" },
  },
  {
    name: "visited-votes-after-unary",
    policy: visited,
    options: {
      ...votes,
      latticeOrder: "after-unary",
      relational: true,
...
writeFileSync(
  out,
  JSON.stringify({
    note: "Adaptive calibration, not final evidence. Integer affine slopes enumerated and intercepts inferred by voting on supplied outputs. Every retained affine fragment is executed and charged. Require three non-collinear supporting inputs. Compare scheduling min/max-cover construction before or after legal unary and macro forward links, or omitting cover construction entirely. Compare against previous random local-triple proposals. The flooded-bank v1 failure is preserved separately. Fixed learned languages are old v4 and replication7, no oracle concepts. All tasks adaptive calibration only, not final evidence. All complete executions and inverse/fitting work charged; point comparisons and wall time also reported. No oracle syntax or hidden checks guide search.",
    rows,
  }),
);
