...
const sorted = [...summaries]
  .filter((r) => r.balancedAuc !== null)
  .sort((a, b) => b.balancedAuc! - a.balancedAuc!);
const report = {
  protocolHash: protocol.protocolHash,
  complete,
  completed: runs.length,
  expected: protocol.variants.length * protocol.languages.length,
  note: "DEVELOPMENT comparison only. Task-cluster bootstrap retains optimizer/library correlation and stratifies by group. Selection over six variants makes winner intervals descriptive, not confirmation. Search work excludes separately reported critic inference; observational wall is not a controlled CPU test. Excluding all historical functions changes this finite generator's mixture, so old calibration rates are not directly comparable.",
  summaries,
  comparisons,
  selectedForFreshConfirmation: complete ? sorted[0].name : null,
  ...
  JSON.stringify(
    {
      ...
      selected: report.selectedForFreshConfirmation,
      summaries,
      comparisons,
    },
    null,
